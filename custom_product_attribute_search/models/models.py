...
class ProductAttributevalue(models.Model):
    ...
    def _variant_name(self, variable_attributes):
        return " ".join([str(v.attribute_id.name)+' '+str(v.name) for v in self.sorted(key=lambda r: r.attribute_id.name) if v.attribute_id in variable_attributes])
        ...
        if name:
            ...
            if not products and operator not in expression.NEGATIVE_TERM_OPERATORS:
                ...
                if not limit or len(products) < limit:
                    ...
                    products += self.search(args + [('id', 'not in', products.ids)], limit=200)
                    products = self.browse([x.id for x in products if x.display_name.lower().startswith(name.lower())])
            elif not products and operator in expression.NEGATIVE_TERM_OPERATORS:
                products = self.search(args + ['&', ('default_code', operator, name)], limit=200)
                products = self.browse([x.id for x in products if x.display_name.lower().startswith(name.lower())])
            # if products:
            #     import pdb;pdb.set_trace()
